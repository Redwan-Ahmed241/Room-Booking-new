...
export default function RoomsPage() {
  ...
  const filterRooms = () => {
    ...
    if (filters.min_price && filters.min_price > 0) {
      filteredRooms = filteredRooms.filter((room) => room.price_per_night >= filters.min_price!)
    }
    if (filters.max_price && filters.max_price < 1000) {
      filteredRooms = filteredRooms.filter((room) => room.price_per_night <= filters.max_price!)
    }
    ...
    // Filter by villa name
    if (filters.villa_name) {
      filteredRooms = filteredRooms.filter((room) =>
        room.villa?.name.toLowerCase().includes(filters.villa_name!.toLowerCase()),
      )
    }
    ...
    if (filters.amenities && filters.amenities.length > 0) {
      filteredRooms = filteredRooms.filter((room) =>
        filters.amenities!.some((amenity) =>
          room.amenities.some((roomAmenity) => roomAmenity.toLowerCase().includes(amenity.toLowerCase())),
        ),
      )
    }
